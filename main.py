...
def analyze_image_gpt4o(file_url):
    try:
        analysis = openai_client.chat.completions.create(
            ...
            messages=[{
                ...
                "content": [
                    ...
                    {"type": "text", "text": "Analyze this image in detail. What product or object is shown? Describe colors, composition, background, and style. Answer in English."}
                ]
            }],
            max_tokens=300
        )
        ...
        return f"product image for NovaX vending machine"
...
def generate_post_content(feedback=None, content_type="post"):
    ...
    if "IMAGE_PROMPT:" in full_text:
        ...
        caption = full_text
        image_prompt = "Modern NovaX vending machine at METU university campus with coffee and snacks, vibrant energetic atmosphere, professional photography"
    return caption, image_prompt


def send_content_to_telegram(caption, image_url, post_id, content_type="post"):
    pending_posts[post_id] = {"caption": caption, "image_url": image_url, "type": content_type}
...
def generate_dm_reply(user_message, username):
    response = claude.messages.create(
        model="claude-sonnet-4-20250514",
        max_tokens=300,
        system="Sen ODTÜ KKK'daki NovaX otomat şirketinin müşteri hizmetleri asistanısın. Türkçe, samimi ve yardımsever cevap ver. Kısa ve net ol.",
...
def telegram_webhook():
    ...
    if message:
        ...
        # Fotoğraf gönderildiyse — analiz et ve profesyonel reklam görseli oluştur
        if photo:
            def process_photo():
                send_telegram("🔍 Görseliniz analiz ediliyor...")
                ...
                analysis = analyze_image_gpt4o(file_url)
                send_telegram(f"✅ Analiz tamamlandı! Profesyonel reklam görseli oluşturuluyor...")

                dalle_prompt = f"""Create a professional advertisement image based on this product: {analysis}
...
- High quality product advertisement look
- NovaX branding feel - energetic and student-friendly"""

                image_url = generate_image_dalle(dalle_prompt)
                caption, _ = generate_post_content(
                    feedback=f"Bu ürün için reklam içeriği yaz: {analysis[:200]}",
                    content_type="post"
                )
                new_id = f"post_{int(time.time())}"
    ...
    if callback:
        ...
        if callback_data.startswith("approve_reply_"):
            ...
            threading.Thread(target=do_regen).start()
